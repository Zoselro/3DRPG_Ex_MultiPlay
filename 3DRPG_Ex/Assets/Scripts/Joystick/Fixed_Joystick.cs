...
public class Fixed_Joystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    void Start()
    {
        ...
        Js_Radius = m_Js_Background.sizeDelta.x * 0.34f; // 중간에 핸들이 벗어나지 못하게 하기 위한 반지름의 34%만 이동시키겠다는 뜻
    ...
    {
        // Js.BackGround 이미지를 마우스로 클릭하는 순간 발생되는 이벤트 함수
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        //1번 매개변수 : 환산 하고싶은 로컬 좌표계 UI의 RectTransform 컴포넌트 객체 대입
        //2번 매개변수 : evnetData.position : 마우스 좌표
        //3번 매개변수 : 터치를 감지할 때 쓰는 카메라 (UI는 보통 Canvas에 연결된 카메라)
        //4번 매개변수 : 변환된 결과. 조이스틱 배경 기준의 위치 (중심을 (0,0)으로 하는 로컬 좌표계로 변환된 값)

        Vector2 clampedPos = Vector2.ClampMagnitude(touchPos, Js_Radius);
        //부모쪽 좌표로 갱신해주기 위한 메서드
        m_Js_Handle.anchoredPosition = clampedPos;
        InputDirection = clampedPos / Js_Radius; // 벡터의 최대 크기는 1.0f 가 될 것임
    ...
    {
        // Js.Background 이미지에서 마우스를 드래그 하다가 손가락을 놓는 순간 발생되는 함수.

        InputDirection = Vector2.zero;
        m_Js_Handle.anchoredPosition = Vector2.zero; // 핸들 초기화 (원 위치로 초기화)
        // 핸들 이미지를 로컬의 중심으로 이동시키겠다는 뜻
